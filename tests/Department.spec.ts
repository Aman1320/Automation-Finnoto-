...
import { checkname, checkrow, checkselected, closebtn, dataclosed, dialogContainer, emptyfield, getButton, locateActive, locatefield, searchBar, selectrow, sortByName } from '../Helper/Department Helper/department.helper';
...
test('Department', async ({ page }) => {
  ...
  const departmentDialog = await dialogContainer(page);
  ...
  const save = await getButton(page,'save');
...
test("Listing", async ({ page }) => {
  ...
  const active = await locateActive(page);
...
test("Details", async ({ page }) => {
  ...
  const dilagbox = await dialogContainer(page);
  ///loop into list
  ...
  const sort = await sortByName(dilagbox);
  await sort.click();
...
test("Hover", async ({ page }) => {
  ...
  const dialog1 = await dialogContainer(page)
  const reset = dialog1.getByRole('link', { name: 'Reset' });
  await reset.click()

  const search = await searchBar(page,"Search column name")
...
test("Table head", async ({ page }) => {
  ...
  //pass the list.
  for (const temp of fieldsToCheck) {
    await checkrow(temp, page);
  }
})
...
test("Search", async ({ page }) => {
  await directlogin(page);
  await page.goto(urlink.department)
  const search = await searchBar(page,"Search ( min: 3 characters )");
  await expect(search).toBeVisible();
...
//inactive from through the listing 
